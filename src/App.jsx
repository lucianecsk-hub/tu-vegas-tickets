...
const PAID = [
  ...
  // SHOWS VISUALES (sin necesidad de inglés)
    ...
    <div style={{
      ...
      {[...Array(30)].map((_,i)=>(
        <div key={i} style={{
          ...
          background:["#2ecc71","#f0c040","#a8d8a8","#c0c0c0","#27ae60"][i%5],
          left:`${(i*3.7)%100}%`, top:`${(i*7.1)%100}%`,
          animation:`pulse ${2+(i%4)}s ease-in-out infinite`,
          animationDelay:`${i*0.18}s`, opacity:.4
        }}/>
      ))}
      ...
      <div style={{maxWidth:"680px",margin:"0 auto",padding:"28px 18px",position:"relative"}}>
        ...
        <div style={{textAlign:"center",marginBottom:"36px",animation:"fadeUp .6s ease"}}>
          <div style={{fontSize:"0.85rem",letterSpacing:"0.45em",color:"#c0392b",marginBottom:"10px",textTransform:"uppercase",fontFamily:"'DM Sans',sans-serif",fontWeight:700}}>
            ✦ TU GUÍA SECRETA ✦
          </div>
          <h1 style={{
            fontSize:"clamp(2rem,7vw,3.4rem)", margin:"0 0 6px",
            fontFamily:"'Playfair Display',serif", fontWeight:700,
            background:"linear-gradient(135deg,#c0392b 0%,#e74c3c 40%,#b8860b 75%,#c0392b 100%)",
            backgroundSize:"200% auto",
            WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent",
            backgroundClip:"text",
            animation:"flicker 9s infinite, shimmer 5s linear infinite",
            lineHeight:1.1, letterSpacing:"0.02em"
          }}>
            TU VEGAS TICKETS
          </h1>
          <p style={{color:"#7a1a1a",fontSize:"0.9rem",margin:0,letterSpacing:"0.12em",fontFamily:"'DM Sans',sans-serif",fontWeight:500}}>
            Lo que Vegas no pone en los folletos
          </p>
        </div>
        ...
        {step===0&&(
          <div style={{animation:"fadeUp .6s ease .2s both"}}>
            <div style={{
              ...
              borderRadius:"22px", padding:"40px 28px", textAlign:"center", marginBottom:"24px",
              boxShadow:"0 4px 24px rgba(26,107,58,.07)"
            }}>
              <div style={{fontSize:"3rem",marginBottom:"20px",animation:"sway 3s ease-in-out infinite"}}>🎰</div>
              <h2 style={{
                fontSize:"1.45rem", color:"#1a1a1a", margin:"0 0 18px",
                fontFamily:"'Playfair Display',serif", fontWeight:700, lineHeight:1.4
              }}>
                Hay un lado de Las Vegas que <em style={{color:"#1a6b3a",fontStyle:"italic"}}>la mayoría nunca descubre.</em>
              </h2>
              <p style={{color:"#333",lineHeight:1.9,margin:"0 0 10px",fontSize:"0.95rem",fontFamily:"'DM Sans',sans-serif"}}>
                Responde unas preguntas y revelaremos tu perfil de viajero — luego construimos un itinerario tan único como tú.
              </p>
              <p style={{color:"#888",fontSize:"0.83rem",margin:0,fontStyle:"italic",fontFamily:"'DM Sans',sans-serif"}}>
                Rápido, personal, gratis.
              </p>
            </div>
            <button onClick={handleNext} style={{
              width:"100%", padding:"20px", borderRadius:"14px", border:"none",
              background:"linear-gradient(135deg,#27ae60,#2ecc71,#f0c040)",
              color:"#0a1a0f", fontSize:"1rem", fontWeight:"700", cursor:"pointer",
              letterSpacing:"0.1em", textTransform:"uppercase",
              boxShadow:"0 8px 32px rgba(46,204,113,.4)",
            ...
            <p style={{textAlign:"center",color:"#1a6b3a",fontSize:"0.82rem",margin:"14px 0 0",fontStyle:"italic",fontFamily:"'DM Sans',sans-serif"}}>
              ✨ Incluye joyas ocultas que solo conocen los locales
            </p>
          </div>
        )}
        ...
        {step===totalSteps+1&&!loading&&(
          <div style={{animation:"fadeUp .5s ease"}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:"6px",marginBottom:"24px"}}>
              {[`${seasonLabels[answers.season]} en Vegas`,`Viaje de ${daysLabels[answers.days]}`].map(b=>(
                <div key={b} style={{background:"rgba(46,204,113,.08)",border:"1px solid rgba(46,204,113,.2)",borderRadius:"20px",padding:"5px 14px",color:"#2ecc71",fontSize:"0.72rem",letterSpacing:"0.06em",fontFamily:"'DM Sans',sans-serif"}}>{b}</div>
              ))}
            </div>
            ...
            {answers.tripType==="family"&&(
              <div style={{background:"rgba(240,192,64,.05)",border:"1px solid rgba(240,192,64,.15)",borderRadius:"16px",padding:"20px",marginBottom:"24px"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                  {[
                    {emoji:"🎂",tip:"La edad importa en Vegas — los menores de 21 no pueden sentarse en barras ni quedarse en casinos. Los niños pueden caminar pero no detenerse."},
                    {emoji:"☀️",tip:"Evita junio-septiembre si es posible. El calor supera los 40°C y agota a los niños rápidamente."},
                    {emoji:"💧",tip:"Lleva botellas de agua siempre. El aire seco del desierto deshidrata a los niños más rápido de lo que esperas."},
                    {emoji:"🎰",tip:"Los niños pueden caminar por los casinos pero no pueden detenerse ni sentarse — incluso con un adulto."},
                    {emoji:"🌅",tip:"Planifica actividades al aire libre por la mañana (antes de las 10am) y regresa al interior durante el pico de calor (12pm-4pm)."},
                    {emoji:"🚗",tip:"Considera alquilar un coche — mucho más fácil con niños pequeños que caminar por el Strip."},
                  ].map((item,i)=>(
